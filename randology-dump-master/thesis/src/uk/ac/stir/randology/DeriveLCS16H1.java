...
/**
 * This experiment calculates Betti_0 numbers in bit-string space using the longest common subsequence metric.
 * @author zak
 *
 */
public class DeriveLCS16H1 {
	...
	class LCS {
		private Double[][] memo = new Double[16][16];
		...
		private double getMemo(int j, int k) {
			if ((j > 16) || (k > 16)) return 0;
			if (memo[j][k] != null) return memo[j][k];
			double result = ((arg1 >>> j) & 1) == ((arg2 >>> k) & 1) ? 1 + getMemo(j+1,k+1) :
	...
	private void testRun(RNG rng) {
		...
		for (int i = 9; i <= 14; i++) {
			int observed = getBetti(barcodes,i,1);
	...
	public void perform() {
		System.out.println("LCS Test, H1, n = 64, deriving phase, START");
