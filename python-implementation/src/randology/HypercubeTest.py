...
def create_filtration_range(n=10, max_value=0.1):
    return np.linspace(0, max_value, n)


class HypercubeTest:

    def __init__(self, runs: int, number_of_points: int, dimension: int, max_simplex_dim: int,
                 filtration_size: int, reference_rng: RNG) -> None:
        self.runs = runs
        ...
        self.filtration_range = create_filtration_range(filtration_size, 0.1)
    ...
    def generate_diagram(self, points: np.array) -> List[np.ndarray]:
        filtration = Rips(maxdim=self.max_simplex_dim, thresh=self.filtration_range[-1])
