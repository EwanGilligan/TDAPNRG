...
public class DeriveHypercube2DXp {
	...
	private double scale = 0.005; // lowest so far: 0.02, for LFSR: 0.15, for Minstd: 0.045
	...
	private double[] testRun(RNG rng) {
		...
		while (j < numberOfPoints) {
			...
			for(int k = 0; k < dimension; k++) {
				correct &= (points[j][k] < 0.5 + (scale/2));
				correct &= (points[j][k] > 0.5 - (scale/2));
			}
	...
	public void perform() {
		System.out.println("Hypercube Test, d = 2, center");
	...
	public static void main(String[] args) {
		new DeriveHypercube2DXp().perform();
	}
